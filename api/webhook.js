 ...
 *   UPSTASH_REDIS_REST_URL   → Upstash Redis URL (isteğe bağlı)
 *   UPSTASH_REDIS_REST_TOKEN → Upstash Redis Token (isteğe bağlı)
 *   CHAT_IDS             → Redis yoksa: "123456,789012" virgülle ayrılmış ID'ler
 */
...
async function handleMessage(chatId, text) {
  ...
  const tf = TF_MAP[cmd];
  if (!tf) return; // Bilinmeyen komut — sessiz geç
